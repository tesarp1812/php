...
foreach ($data as $key => $value) {
    if (str_contains($key, "SMA")) {
        $value["name"] = $key;
        array_push($sma, $value);
    }
    if (str_contains($key, "SMK")) {
        $value["name"] = $key;
        array_push($smk, $value);
    }
    if (str_contains($key, "SMP")) {
        $value["name"] = $key;
        array_push($smp, $value);
    }

    foreach ($value['siswa'] as $sub_key => $sub_val) {
        array_push($siswa, $sub_val);
    }
...
<form>
    <select  onchange="tampilkan(this.value)">
...
<div id="SMA" style="display: none">
    <?php foreach ($sma as $jar ) { ?>
        <div id="SMA">
            <div>
                <label>NAMA SEKOLAH</label>
                <div>
                    <input type="text" value="<?php echo $jar['name'] ?>">
                </div>
            </div>
            <div>
                <label>TELEPON</label>
                <div>
                    <input type="text" value="<?php echo $jar['tlp'] ?>">
                </div>
            </div>
            <div>
                <label>ALAMAT SEKOLAH</label>
                <div>
                    <input type="text" value="<?php echo $jar['alamat'] ?>">
                </div>
            </div>
            <div id="siswa">
...
<script>
    function tampilkan(value) {
        let array = ["SMA", "SMK", "SMP"];
        for (let index = 0; index < array.length; index++) {
            if (value == array[index]) {
                document.getElementById(array[index]).style.display = "block";
            } else {
